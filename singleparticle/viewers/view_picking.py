...
class PickingDialog(ToolbarListDialog):
            ...
            if ps is not None:
                if self.spacing is None:
                    ...
                    if self.spacing == ps:
                        ...
                        raise ValueError(
                            f"Images from {provider.fluoimagesList} don't have the same pixel size."
                            f"Got {self.spacing} and {ps}."
                        )
    ...
    def lanchNapariForFluoImage(self, im: FluoImage):
        ...
        args = [path, csv_path, "--size", f"{self.size}", "--spacing", str(self.spacing[1]), str(self.spacing[0]), str(self.spacing[0])]
