...
def reconstruction_L2(volumes: torch.Tensor, psf: torch.Tensor, poses: torch.Tensor, lambda_: torch.Tensor) -> torch.Tensor:
    ...
    for start, end in utils_memory.split_batch_func(
...
def get_planes_intersection(self, s1, s2):
    ...
    d = np.array([-equation1[3], -equation2[3], 0.]).reshape(3,1)
...
def line_intersect_surface(self, line, surface):
    ...
    projector = lambda x: np.array([np.dot(plane_orthonormal_basis[0], x-plane_basis_position), np.dot(plane_orthonormal_basis[1], x-plane_basis_position)])
    projected_line = (
        projector(line[1]+10*line[0])-projector(line[1]),
        projector(line[1])
    )
    projected_surface = np.array([projector(surface[0]), projector(surface[1]), projector(surface[2])])
...
def surfaces_intersect(self, s1, s2):
    ...
    if ret1 and ret2:
        return ((tmin1 <= tmax2) and (tmin2 <= tmin1)) or ((tmin2 <= tmax1) and (tmin1 <= tmin2))
    else:
        return False
...
def pointcloud_intersect(self, corners1, corners2):
    ...
    intersections = [self.surfaces_intersect(s1, s2) for s1, s2 in
        itertools.product(surfaces1, surfaces2)
    ]
