...
def build_particle(patches, tree, R, ty, tx):
    ...
    if type(tree[2]) is list:
        m = tree[0]
        p = patches[tree[1]]
        return transform_patch(p + (m-1)*build_particle(patches, tree[2], r_, ty_, tx_), R, ty, tx) / m
    else:
        assert tree[0] == 2
        r_, ty_, tx_ = tree[3]
...
def fill_params(params, tree, R, ty, tx):
    ...
    R_rad = R * np.pi / 180
    if type(tree[2]) is list:
        fill_params(params, tree[2], R+r_, tx*np.sin(R_rad)+ty*np.cos(R_rad)-ty_, tx*np.cos(R_rad)-ty*np.sin(R_rad)-tx_)
        #fill_params(params, tree[2], R+r_, ty+ty_, tx+tx_)
    else:
        params[tree[2]] = ((R+r_)%360,  tx*np.sin(R_rad)+ty*np.cos(R_rad)-ty_, tx*np.cos(R_rad)-ty*np.sin(R_rad)-tx_)
...
def iterative_alignment(particles, epsilon=1e-2, max_iter=20):
    ...
    # Initialize the tree
    tree = [2, order[0], order[1], get_transform(patches_padded_centered[order[0]], patches_padded[order[1]])]
    #plt.subplot(141)
    ...
    while distance.sum() > epsilon and j < max_iter:
        ...
        for i in range(N):
            ...
            transformed_p = transform_patch(p, *patches_param_centered[order[i]])# if last_transforms[order[i]] is None else last_transforms[order[i]]
...
def iterative_suppression(patches, n):
    ...
    for i in range(I):
        ...
        if i < I-1: # not last iteration
            mask_keep[mask_keep] = std <= np.median(std) # we keep half of the particles
        else:
...
def get_first_euler_angle_side(side_particles, top_particles, psf, lambda_=1e-2):
    ...
    side_centers_of_mass = np.array(list(map(lambda x: ndii.center_of_mass(x)[0], side_particles.cpu().numpy()))) - side_particles.size(1)/2
    side_poses[:, 3] = as_tensor(side_centers_of_mass)
    top_centers_of_mass = np.array(list(map(lambda x: ndii.center_of_mass(x)[0], top_particles.cpu().numpy()))) - side_particles.size(1)/2
    ...
    t3 = time()
    recon, den = reconstruction_L2(particles, psf, poses_expanded, lambda_[None].repeat(M))
    ...
    for i in range(recon.size(0)):
        patches_transformed = inverse_affine_transform(particles[:,None], poses_expanded[i])[:,0]
        patches_transformed_fft = torch.fft.fftn(
            torch.fft.fftshift(patches_transformed, dim=(1,2,3)),
            dim=(1,2,3)
        )
        #patches_transformed_fft = torch.fft.fftn(
        #    patches_transformed,
        ...
        poses_no_tvec[:, 3:] = 0
        psf_transformed = inverse_affine_transform(psf.repeat(poses_expanded.size(1),1,1,1,1), poses_no_tvec)[:,0]
        

        psf_transformed_fft = torch.fft.fftn(
            torch.fft.fftshift(
                pad_to_size(psf_transformed, psf_transformed.shape[:1]+particles.shape[-3:]), dim=(1,2,3)
            ),
            dim=(1,2,3)
        )
        #psf_transformed_fft = torch.fft.fftn(
        ...
        MSE[i] = (recons_conv_fft - patches_transformed_fft).square().sum() / particles.size(0)
