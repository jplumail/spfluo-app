...
class Trainset(Dataset):
    ...
    def __init__(self, paths: List[str], labels: np.ndarray, patch_size: Tuple[int],
                 extension: str, augment: float, training: bool, size: int=None) -> None:
        super().__init__()
        self.paths      = paths
        self.labels     = labels
...
def make_data(
    ...
    if unsupervised:
        ...
        if extension == 'npz':
            ...
            train_views = load_annotations(os.path.join(rootdir, 'train', 'train_views.csv'))
    ...
    if selected_particles is None:
        ...
        width = int(np.ceil(np.log10(len(os.listdir(os.path.join(datadir, "positive"))))))
        train_paths = list(map(lambda x: os.path.join(datadir, "positive", x[1].replace('.'+extension,'_'+str(x[0]).zfill(width)+'.npz')), picked_particles))
    ...
    val_set   = Trainset(val_paths, val_views, patch_size, extension, augment=0, training=False)
    # loaders
...
def train(
    ...
    data_params  = [rootdir, output_dir, extension, batch_size,
                    num_workers, shuffle, augment, size, patch_size]
    ...
    if pretrain:
        ...
        data = make_data(*data_params, None, True, None, crop_dir)  # num_pos_samples=None <=> use all samples
        model = make_model(*train_params, *model_params, *[True, *ae_params[1:]])  # ae = True
