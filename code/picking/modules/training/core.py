...
class DataTracker:
    ...
    def __init__(self, names: List[str], init_best: float=0.) -> None:
        self.names   = names
        self.total   = 0
        self.current = {}
...
class F1Score(DataTracker):
    ...
    def step(self, outputs: torch.Tensor, targets: torch.Tensor) -> None:
        ...
        self.correct_true   += (targets[target_true] == predictions[target_true]).sum().item()
    ...
    def reduce(self) -> None:
        ...
        precision = self.correct_true / self.predicted_true if self.predicted_true > 0 else 0
...
class Logger:
    ...
    def step(self, losses: List[Tensor], total_loss: Tensor, outputs: Tensor, targets: Tensor):
    ...
    def to_tensorboard(self, training: bool) -> None:
        ...
        for name, loss in self.losses.current.items():
            self.writer.add_scalar(f'{name.capitalize()} Loss/{stage}', loss, self.epoch)
...
def make_model(
    ...
    # 2. Network
    kwargs = dict(
        dim=dim, num_classes=num_classes, width_factor=width_factor, depth_factor=depth_factor
    )
    ...
    if num_classes == 3:  # tilt
        ...
        # pi is None <=> FS or PU
        train_criteria = [BinomialGE(pi=pi) if pi is not None else torch.nn.BCEWithLogitsLoss()]
    ...
    multi_task_loss_kwargs = {
        'eta': len(train_criteria) * [eta], 'learn_eta': learn_eta, 'device': device
    }
    ...
    # 6. Optimizer
    adam_params = {'lr': lr, 'betas': (0.9, 0.999), 'weight_decay': 0.01, 'amsgrad': False}
    ...
    # scheduler = None
    model = Model(device, network, decoder, optimizer, scheduler, train_criterion, val_criterion)
...
def step(x: Tensor, y: Tensor, model: Model, logger: Logger, training: bool) -> None:
    ...
    if model.decoder is not None:
        model.val_criterion.eta = model.train_criterion.eta  # in case eta is being learnt
...
def half_epoch(dataloader: DataLoader, model: Model, logger: Logger, training=True) -> Model:
