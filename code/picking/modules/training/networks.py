...
class MBConv1(MBConvN):
    def __init__(self, n_in, n_out, kernel_size=3, stride=1, r=24, p=0, dim=2):
        super().__init__(n_in, n_out, expand_factor=1, kernel_size=kernel_size,
                         stride=stride, r=r, p=p, dim=dim)


class MBConv6(MBConvN):
    def __init__(self, n_in, n_out, kernel_size=3, stride=1, r=24, p=0, dim=2):
        super().__init__(n_in, n_out, expand_factor=6, kernel_size=kernel_size,
                         stride=stride, r=r, p=p, dim=dim)
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, in_channels=1, width_factor=1, depth_factor=1, num_classes=1000,
                 mode=None, dim=2):
        super().__init__()
        ...
        stage_params  = [(scaled_widths[i], scaled_widths[i + 1], scaled_depths[i],
                          layers[i], kernel_sizes[i], strides[i],
    ...
    @staticmethod
    def create_stage(n_in, n_out, num_layers, layer_type, kernel_size, stride, r, p, dim):
        ...
        layers += [layer_type(n_out, n_out, **common_params) for _ in range(num_layers - 1)]
