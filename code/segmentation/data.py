...
class _AbstractBlock(torch.nn.Module):
    ...
    def get_info(self):
        return dict(map(lambda x: (x[0], x[1].clone().float().squeeze().numpy()), self.named_parameters(recurse=False)))
    
    def update_info(self, info):
        if self.__class__.__name__ not in info: info[self.__class__.__name__] = {}
...
class _AbstractAugment(_AbstractBlock):
    ...
    def sample_params(self):
        return dict(map(lambda x: (x[0], x[1].sample()), self.named_parameters(recurse=False)))
...
class _RandomParticle(_Generator):
    def __init__(self):
        ...
        self.params['MakeHoles']['args'] = dict(
            nb_holes=DiscreteUniform(0, 10),
            mean_ratio=0.5, std_ratio=0.25
        )
...
class _DefaultGenerator(_Generator):
    def __init__(self):
        ...
        self.params['AddOutlierClusters']['args']['nb_clusters'] = DiscreteUniform(0, 10)
        self.params['AddGaussianNoise']['args'] = dict(
            target_snr=Normal(loc=15,scale=2),
            gaussian_noise_mean=Uniform()
        )
...
class NoAugGenerator(_DefaultGenerator):
    def __init__(self, psf_coef=(.1,.1,.1)):
        super().__init__()
        aug_blocks = [
            'Shrink',
            'Translate',
        ...
        self.params['ApplyPSF']['args'].update(dict(
            cov=psf_coef,
...
class SingleParticleGenerator_translation_holes(_DefaultGenerator):
    def __init__(self, image_shape=(32,32,32), voxelise_res=30, psf_coef=(.1,.1,.1), nb_holes=10, tvec_norm=Normal(loc=100, scale=10)):
        super().__init__()
        aug_blocks = [
            'Shrink',
            'DropPointcloud',
        ...
        if nb_holes == 0:
            ...
            self.params['MakeHoles']['args'] = dict(
            nb_holes=DiscreteUniform(0, nb_holes),
            mean_ratio=0.5, std_ratio=0.25
        )
...
class RandomParticle_variableSize(_RandomParticle):
    def __init__(self, low, high=None):
        ...
        self.params['CreatePointcloud']['args'].update(dict(
            l=Uniform(low, high),
        ))
        self.params['Shrink']['args'].update(dict(
            factor=1.0,
        ))
        self.build_module()
...
class Generator_variableSize(_DefaultGenerator):
    def __init__(self, low, high=None):
        ...
        self.params['AddRandomParticle']['args'].update(dict(
            create_particle=RandomParticle_variableSize(low, high)
        ))
...
class ISIMParticle(_RandomParticle):
    def __init__(self):
        ...
        self.params['Voxelise']['args'].update({'img_shape': (64,64,64), 'resolution': 20})
        
        self.params['Shrink']['args']['factor'] = Normal(loc=1, scale=0.1)

        self.params['MakeHoles']['args'] = dict(
            nb_holes=DiscreteUniform(100, 200),
            mean_ratio=0.1, std_ratio=0.02
        )
...
class ISIMGenerator(_DefaultGenerator):
    def __init__(self):
        ...
        self.params['DropPointcloud']['p'] = 0.
        self.params['AddRandomParticle']['p'] = 1.
...
class Translate(_AbstractAugment):
    def __init__(self, tvec_norm=0):
        ...
        if callable(tvec_norm):
            tvec_sampling_strategy = SamplingStrategy(lambda: get_random_3d_vector(tvec_norm()))
        else:
            tvec_sampling_strategy = SamplingStrategy(lambda: get_random_3d_vector(tvec_norm))
...
class Rotate(_AbstractAugment):
    ...
    def transform(self, data):
        ...
        rot = Rotation.from_euler('ZXZ', self.rotation_angles.cpu().numpy(), degrees=True)
        rot_mat = torch.as_tensor(rot.as_matrix(), dtype=pointcloud.dtype, device=pointcloud.device)
...
class MakeHoles(_AbstractAugment):
    ...
    def get_center_and_radius(self, pointcloud: torch.Tensor) -> Tuple[torch.Tensor, float]:
    ...
    def uniform_in_sphere(self, center: torch.Tensor, radius: float, N: int=1) -> torch.Tensor:
    ...
    def transform(self, data: Tuple[torch.Tensor]) -> torch.Tensor:
        ...
        if nb_holes > 0:
            sigma = torch.clamp(mean_scaled + torch.randn(size=(nb_holes,), device=std_scaled.device) * std_scaled, min=1e-3)
            cov = (sigma[:,None,None] ** 2) * torch.eye(3, device=sigma.device)
            holes_distributions = MultivariateNormal(hole_centers, covariance_matrix=cov) # nb_holes multivariate normal distributions
            prob = torch.exp(holes_distributions.log_prob(pointcloud_noisy[:,None])) / torch.exp(holes_distributions.log_prob(hole_centers)) # shape (N, nb_holes)
...
class Voxelise(_AbstractTransform):
    def __init__(self, center=(0,0,0), img_shape=(65,65,65), resolution=30, bandwidth=10, epsilon=1e-3, device=torch.device('cpu'), dtype=torch.float32):
        super().__init__()
        ...
        start = torch.as_tensor(center).cpu() - resolution * torch.as_tensor(img_shape).cpu() / 2
        end = start + resolution * torch.as_tensor(img_shape).cpu()

        target_points = torch.meshgrid(
            *[torch.linspace(st, e, sh, device=self.device) for st, e, sh in zip(start, end, img_shape)],
            indexing='ij'
        )
        self.target_points = torch.stack(target_points, dim=0)
    
    @staticmethod
    def get_FOV(pointcloud: Union[torch.Tensor, np.ndarray]) -> Tuple[Tuple[float,float],...]:
        ...
        """
        return tuple([(pointcloud[:, i].min(), pointcloud[:, i].max()) for i in range(pointcloud.shape[1])])
    ...
    def get_densities(self, pointcloud):
        if self.device.type == 'cuda':
            return (
                ...
                * torch.sum(
                    torch.exp(
                        -((pointcloud[:,:,None,None,None] - self.target_points[None].to(pointcloud.device)) ** 2) \
                            .sum(dim=1) / (2 * self.bandwidth.to(pointcloud.device) ** 2)
                    ),
                    dim=0,
                )
            )
            ...
            figtree_kwargs = {'bandwidth': self.bandwidth.cpu().numpy(), 'epsilon': self.epsilon.cpu().numpy()}
            if pointcloud.size(0) > 0:
                densities = figtree(pointcloud.cpu().numpy().astype(np.float64), self.target_points.cpu().numpy().astype(np.float64).transpose(1,2,3,0).reshape(-1,3), weights, **figtree_kwargs)
                div = densities.max() - densities.min()
                if div > 0:
                    densities = (densities - densities.min()) / div
            else:
                densities = np.zeros(self.target_points.shape[1:])
            densities = torch.as_tensor(densities.reshape(self.target_points.shape[1:]), device=pointcloud.device, dtype=torch.float64).type(self.dtype)
            return densities
...
class ApplyPSF(_AbstractAugment):
    def __init__(
        ...
        if psf_path is not None and os.path.exists(psf_path):
            ...
            self.psf = torch.as_tensor(load_array(psf_path), device='cpu').type(torch.float32)
        else:
            self.psf_path = None
            self.psf = None
            self.psf_size = Parameter(psf_size)
            if cov_coef_sampling_strategy is not None and anisotropy_sampling_strategy is not None:
                cov_sampling = SamplingStrategy(lambda: self.sample_cov(cov_coef_sampling_strategy, anisotropy_sampling_strategy))
                self.cov_coef = Parameter(
                    cov_sampling,
                    size=(3,),
                    min=0.1
                )
    ...
    def sample_cov(self, cov_sampling_strategy, anisotropy_sampling_strategy):
        ...
        a = max(1., anisotropy_sampling_strategy())
    ...
    def transform(self, data):
        ...
        img = F.conv3d(img[None,None].to(self.device), psf[None,None], padding='same')[0,0].to(img.device)
        #div = img.max() - img.min()
        #if div > 0:
        #    img = (img - img.min()) / div
...
class AddRandomParticle(_AbstractAugment):
    ...
    def get_translation_vec(self, reference_mask, mask_to_move, direction, distance):
        ...
        while np.abs(upper_bound - lower_bound) > 1:
            ...
            mask_moved = mask_moved_padded[D-tvec[0]:D-tvec[0]+D, H-tvec[1]:H-tvec[1]+H, W-tvec[2]:W-tvec[2]+W]
    ...
    def translate(self, img, tvec):
        ...
        return img_padded[D-tvec[0]:D-tvec[0]+D, H-tvec[1]:H-tvec[1]+H, W-tvec[2]:W-tvec[2]+W]
...
class AddOutlierClusters(_AbstractAugment):
    ...
    def sample_outliers_clusters(self, img):
        if self.nb_clusters > 0:
            radius = torch.stack([torch.randint(r[0], r[1], size=(int(self.nb_clusters),)) for r in self.radius_ranges], dim=1)
            cov = radius[:,None]**0.5 * torch.eye(3)
            center = torch.as_tensor([[torch.randint(0, img.shape[i], size=(1,)) for i in range(radius.size(1))] for j in range(radius.size(0))]).type(cov.dtype)
            ...
            outliers_clusters = [cluster[:n] for cluster, n in zip(outliers_clusters, nb_points)]
            masks = [((cluster > 0) * (cluster < torch.as_tensor(img.shape))).prod(dim=1).type(torch.bool) for cluster in outliers_clusters]
            outliers_clusters = [cluster[mask] for cluster, mask in zip(outliers_clusters, masks)]
    ...
    def transform(self, data):
        ...
        for cluster in outliers_clusters:
            intensities = (high - low) * torch.rand((len(cluster),), device=img.device, dtype=img.dtype) + low
...
class AddGaussianNoise(_AbstractAugment):
    ...
    def transform(self, data):
        ...
        noise = torch.normal(
            float(self.gaussian_noise_mean),
            float(noise_average)**0.5,
            size=img.shape
        ).to(img.device)
        img += noise
...
class LoadPointcloud(_AbstractTransform):
    def __init__(self, pointcloud_path="/data1/stage_Luc/inputs/sample_centriole_point_cloud.csv"):
        super().__init__()
...
class CreatePointcloud(_AbstractAugment):
    ...
    def create_centriole(self):
        ...
        pc = simu_cylinder_rev(
            get_radius(x, self.a.cpu().numpy(), self.b.cpu().numpy(), self.c.cpu().numpy()),
            self.r.cpu().numpy(),
            self.l.cpu().numpy(),
            self.s.cpu().numpy(),
            self.N,
...
class HardCopyDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, **kwargs):
        ...
        self.data = [(deepcopy(im[0]),deepcopy(mask[0]),deepcopy(unbatch_dict(info)[0])) for im, mask, info in loader]
