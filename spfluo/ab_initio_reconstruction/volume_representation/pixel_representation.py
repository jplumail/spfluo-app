...
class Fourier_pixel_representation:
    ...
    def get_energy(
        ...
        if known_trans:
            view_rotated_fft = fft.fftn(
                rotation(
                    view,
                    rot_mat.T,
                    trans_vec=-rot_mat.T @ trans_vec,
                    order=interp_order,
                )[0]
            )
        else:
            view_rotated_fft = fft.fftn(
                rotation(view, rot_mat.T, order=interp_order)[0]
            )
            shift, _, _ = phase_cross_correlation(
                psf_rotated_fft * self.volume_fourier,
                view_rotated_fft,
                space="fourier",
                upsample_factor=10,
                normalization=None,
        ...
        energy = np.linalg.norm(
            psf_rotated_fft * self.volume_fourier - view_rotated_fft
        ) ** 2 / (self.size**self.nb_dim)
    ...
    def get_energy_gpu(
        ...
        if known_trans:
            ...
            view_rotated_fft = cp.fft.fftn(
                rotation_gpu(view, rot_mat.T, order=interp_order)[0]
            )
            shift, _, _ = phase_cross_correlation_gpu(
                psf_rotated_fft * self.volume_fourier_gpu,
                view_rotated_fft,
                space="fourier",
                upsample_factor=10,
                normalization=None,
            ...
            if save_shift:
                recorded_shifts[view_idx].append(-rot_mat @ shift)
        energy = cp.linalg.norm(
            psf_rotated_fft * self.volume_fourier_gpu - view_rotated_fft
        ) ** 2 / (self.size**self.nb_dim)
        variables_used_to_compute_gradient = (psf_rotated_fft, view_rotated_fft)
        return energy, variables_used_to_compute_gradient

    def get_energy_gpu_pytorch(
        self,
        rot_mats,
        trans_vec,
        view,
        ...
        if known_trans:
            ...
            _, shift = phase_cross_correlation_gpu_pytorch(
                psfs_rotated_fft * self.volume_fourier_gpu_torch.to(rot_mats.device),
                view_rotated_fft,
                nb_spatial_dims=3,
                upsample_factor=10,
                normalization=None,
            ...
            view_rotated_fft = fourier_shift_gpu_pytorch(
                view_rotated_fft, shift, nb_spatial_dims=3
            )
            if save_shift:
                recorded_shifts[view_idx].append(-rot_mats.permute((0, 2, 1)) @ shift)
        energy = torch.linalg.norm(
            (
                psfs_rotated_fft * self.volume_fourier_gpu_torch.to(rot_mats.device)
                - view_rotated_fft
            ).view(psfs_rotated_fft.size(0), -1),
            dim=1,
        ) ** 2 / (self.size**self.nb_dim)
    ...
    def get_energy_2(self, rot_mat, trans_vec, view_fft, known_trans=True):
        ...
        if not known_trans:
            ...
            shift = trans_vec
        translated_convolved_rotated_volume = fourier_shift(
            convolved_rotated_volume, shift
        )
        energy = np.linalg.norm(translated_convolved_rotated_volume - view_fft) ** 2 / (
            self.size * self.nb_dim
        )
        return energy

    def compute_grad(
        self, rot_mat, trans_vec, views, known_trans, view_idx, interp_order=3
    ):
        view = views[view_idx]
        energy, variables_used_to_compute_gradient = self.get_energy(
            rot_mat,
            trans_vec,
            view,
            None,
    ...
    def squared_variations_regularization(self):
        ...
        TV = (
            np.mean((self.volume_fourier - v1) ** 2)
            + np.mean((self.volume_fourier - v2) ** 2)
            + np.mean((self.volume_fourier - v3) ** 2)
        )
        gradient_TV = (
            2
            * self.volume_fourier
            * (3 * self.volume_fourier - v1 - v2 - v3)
            / self.size**3
        )
