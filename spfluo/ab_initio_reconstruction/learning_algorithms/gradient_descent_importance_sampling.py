...
def gd_importance_sampling_3d(volume_representation, uniform_sphere_discretization, true_trans_vecs, views, imp_distrs_axes,
                              imp_distrs_rot, unif_prop,  unif_prop_min, params_learning_alg, known_trans, output_dir, ground_truth=None,
                              file_names=None, folder_views_selected=None, gpu=None):
    unif_prop_axes, unif_prop_rot = unif_prop
    epoch_length = params_learning_alg.epoch_length if params_learning_alg.epoch_length is not None else len(views)
    ...
    while itr < params_learning_alg.N_iter_max and (not stopping_criteria(recorded_energies, params_learning_alg.eps)):
        ...
        if params_learning_alg.random_sampling:
            ...
            last_energies = np.array(
                [energies_each_view[v][-1] if len(energies_each_view[v]) > 0 else np.inf for v in range(len(views))]
            )
            m = last_energies.max()
            if np.isinf(m):
                weights = np.array([1. if np.isinf(e) else 0. for e in last_energies])
                weights /= weights.sum()
            else:
                last_energies_centered = last_energies - m
                weights = np.exp(params_learning_alg.beta_sampling*last_energies_centered)/np.exp(params_learning_alg.beta_sampling*last_energies_centered).sum() # softmax
        ...
        batches = [chosen_views[i*bs:(i+1)*bs] for i in range(epoch_length//bs)]
        ...
        for batch in pbar2:
            ...
            for v in batch:
                ...
                if gpu == 'pytorch' or gpu == 'cucim':
                    rot_vecs = np.stack(np.broadcast_arrays(thetas[indices_axes][:,None], phis[indices_axes][:,None], psis[indices_rot]), axis=-1) # Na x Nr x 3
                    rot_mats = R.from_euler('zxz', rot_vecs.reshape(-1,3), degrees=True).as_matrix() # (Na*Nr) x 3 x 3
                if gpu == 'pytorch':
                    rot_mats_gpu, true_trans_vec_gpu, view_gpu = \
                        map(
                            lambda x: torch.as_tensor(x.astype(params_learning_alg.dtype), device='cuda') if x is not None else None,
                            [rot_mats, true_trans_vec, views[v]]
                        )
                    energies, _ = volume_representation. \
                        get_energy_gpu_pytorch(rot_mats_gpu, true_trans_vec_gpu, view_gpu, None, None, False, known_trans, interp_order=params_learning_alg.interp_order)
                    energies = energies.cpu().numpy().reshape(len(indices_axes), len(indices_rot))
                elif gpu == 'cucim':
                    rot_mats_gpu, true_trans_vec_gpu, view_gpu = \
                        map(
                            lambda x: cp.array(x.astype(params_learning_alg.dtype)) if x is not None else None,
                            [rot_mats, true_trans_vec, views[v]]
                        )
                    rot_mats_gpu = rot_mats_gpu.reshape(params_learning_alg.N_axes, params_learning_alg.N_rot, 3, 3)
                    energies = cp.array(energies)
                    for j, idx_axes in enumerate(indices_axes):
                        for k, idx_rot in enumerate(indices_rot):
                            energies[j, k], _ = volume_representation.get_energy_gpu(
                                rot_mats_gpu[j, k],
                                true_trans_vec, view_gpu,
                                None, None, False, known_trans,
                    ...
                    for j, idx_axes in enumerate(indices_axes):
                        for k, idx_rot in enumerate(indices_rot):
                            ...
                            rot_mat = get_rotation_matrix(rot_vec, params_learning_alg.convention).astype(params_learning_alg.dtype)
                            energy, _ = volume_representation. \
                ...
                grad, energy = volume_representation.compute_grad(
                    rot_mat, true_trans_vec, views, known_trans, v,
                    interp_order=params_learning_alg.interp_order
                )
                ...
                update_imp_distr(imp_distrs_axes, phi_axes, K_axes,unif_prop_axes, M_axes, v)
                update_imp_distr(imp_distrs_rot, phi_rot, K_rot, unif_prop_rot, M_rot, v)
            ...
            volume_representation.gd_step(grad, params_learning_alg.lr, params_learning_alg.reg_coeff)
    ...
    shutil.copyfile(os.path.join(sub_dir, f'recons_epoch_{itr}.tif'), os.path.join(sub_dir, f'final_recons.tif'))
    pbar.close()
    write_array_csv(np.array(ssims), f'{output_dir}/ssims.csv')

    np.save(os.path.join(output_dir, 'energies_each_view.npy'), np.array(energies_each_view))
    np.save(os.path.join(output_dir, 'distributions_rot.npy'), np.stack(imp_distrs_rot_recorded, axis=0))
    np.save(os.path.join(output_dir, 'distributions_axes.npy'), np.stack(imp_distrs_axes_recorded, axis=0))
