...
def translation(image, trans_vec, order=interp_order):
    ...
    return affine_transform(image, np.eye(nb_dim), -trans_vec, order=order, mode='nearest')
...
def rotation(volume, rot_mat, order=interp_order, trans_vec=None):
    ...
    rotated = affine_transform(volume, rot_mat.T, c-rot_mat.T @ (c + trans_vec), order=order, mode='constant')
...
def rotation_gpu_pytorch(volume, rot_mat, order=1, trans_vec=None):
    ...
    rotated = affine_transform_pytorch(volume, rot_mat.transpose(1,2), (c - rot_mat.transpose(1,2) @ (c + trans_vec))[...,0], order=order, mode='zeros')
    return rotated, rot_mat
