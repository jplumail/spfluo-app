from .get_poses import load_views, load_patches, normalize_patches, iterative_suppression
...
def main(args):
    ...
    mask_keep_side, params_side, avrg_particle_side, variance_map_side = iterative_suppression(patches_2D[views==1], args.number_side)
    mask_keep_top, params_top, avrg_particle_top, variance_map_top = iterative_suppression(patches_2D[views==0], args.number_top)

    params = np.zeros((patches.shape[0], 6)) # shape (N, (euler1, euler2, euler3, tz, ty, tx))
