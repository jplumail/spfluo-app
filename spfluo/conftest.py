...
import torch
from spfluo.picking.modules.pretraining.generate.tests.test_data_generator import generated_data, psf_array, groundtruth_array, particles, generated_particles_dir
...
def volumes_and_poses(generated_data: Tuple[Any, Any, Dict[str, Dict[str, np.ndarray]]]) -> Tuple[np.ndarray, np.ndarray]:
...
@pytest.fixture(scope="session")
def generated_data_arrays(volumes_and_poses: Tuple[np.ndarray, np.ndarray], psf_array: np.ndarray, groundtruth_array: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
...
@pytest.fixture(scope="session")
def generated_data_pytorch(generated_data_arrays: Tuple[np.ndarray, ...]) -> Tuple[torch.Tensor, ...]:
