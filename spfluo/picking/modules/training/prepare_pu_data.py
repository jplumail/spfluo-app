...
def recenter_if_needed(dim: Tuple[int], shape: Tuple[int], z: int, y: int, x: int) -> Tuple[int]:
    d, h, w = dim
    D, H, W = shape
...
def make_P_mask(shape: Tuple[int], centers: np.ndarray, radius: int=10) -> np.ndarray:
    ...
    for center in centers:
        x_min, x_max, y_min, y_max, z_min, z_max = recenter_if_needed(dim, shape, *center)
...
def prepare_pu_data(
    ...
    if save_selected_particles:
        with open(os.path.join(output_dir, 'selected_particles.pickle'), 'wb') as file:
            ann_to_save = np.concatenate((num[annotations_range[:num_pos_samples], None], annotations), axis=1)
...
class PUSampler(Sampler):
    ...
    def __init__(self, P_masks: List[np.ndarray]=[], U_masks: List[np.ndarray]=[],
                 pi: float=0.5, batch_size: int=64, epoch_size: int=None) -> None:
    ...
    def enumerate_positive_coordinates(masks: np.ndarray) -> np.ndarray:
        ...
        positive_coordinates = np.zeros(size, dtype=[('image', np.uint32), ('coord', np.uint32)])
        index = 0
        for mask_index, mask in enumerate(masks):
            positive_indices     = np.flatnonzero(mask)
            num_positive_indices = len(positive_indices)
            positive_coordinates['image'][index:index + num_positive_indices] = mask_index
            positive_coordinates['coord'][index:index + num_positive_indices] = positive_indices
...
def estimate_pi(
    ...
    expected_num_particles_per_image: int,
    radius: int
) -> float:
    num_images = len(U_masks)
    num_pixels_per_particle = get_num_pixels_per_particle(radius)
    expected_P_size = num_pixels_per_particle * expected_num_particles_per_image * num_images
...
def make_dataloaders(
    ...
    args = [datadir, output_dir, csv_name, extension, radius,
            shuffle, num_pos_samples, load_u_masks, True]
    ...
    kwargs = {'batch_size': batch_size, 'num_workers': num_workers, 'collate_fn': collate_fn}
