...
class BinomialGE(torch.nn.Module):
    ...
    def __init__(self, pi: float=None, slack: float=1., entropy_penalty: float=0.) -> None:
    ...
    def forward(self, outputs: Tensor, targets: Tensor) -> Tensor:
        ...
        count_vector = torch.arange(0, N + 1, device=targets.device, dtype=torch.float32)
        # below, 1e-10 is only a small epsilon to prevent NaN
        q_discrete = (-0.5 * (q_mu - count_vector) ** 2 / (q_var + 1e-10)).softmax(dim=0)
...
class MultiTaskLoss(torch.nn.Module):
    ...
    def __init__(self, criteria: Sequence[torch.nn.Module], eta: Sequence[float]=[0],
                 learn_eta: bool=False, device: torch.device=torch.device('cpu')) -> None:
    ...
    def forward(self, outputs: Sequence[Tensor], targets: Sequence[Tensor]) -> Sequence[Tensor]:
        loss = [criterion(o, y) for criterion, o, y in zip(self.criteria, outputs, targets)]
