...
def BatchNormNd(*args, dim=2, **kwargs):
    return (
        nn.BatchNorm2d(*args, **kwargs) if dim == 2 else nn.BatchNorm3d(*args, **kwargs)
    )
...
class ConvBnAct(nn.Sequential):
    ...
    def __init__(
        ...
        self.add_module(
            "bn", BatchNormNd(n_out, dim=dim, momentum=0.99) if bn else nn.Identity()
        )
...
class MBConvN(nn.Module):
    ...
    def __init__(
        self, n_in, n_out, expand_factor, kernel_size=3, stride=1, r=24, p=0, dim=2
    ):
        super().__init__()
...
class MBConv1(MBConvN):
    def __init__(self, n_in, n_out, kernel_size=3, stride=1, r=24, p=0, dim=2):
        super().__init__(
            n_in,
            n_out,
            expand_factor=1,
...
class EfficientNet(nn.Module):
    ...
    's' and 'xs'. Those are almost equals to b0 except for one or two stride(s), respectively
    for 's' and 'xs', that are 1 instead of 2 in order to allow smaller inputs.
    It can handle 2D or 3D data through the 'dim' argument.
    """
    ...
    def __init__(
        ...
        stem_params = {"stride": 1 if mode == "xs" else 2, "padding": 1, "dim": dim}
        self.stem = ConvBnAct(in_channels, scaled_widths[0], **stem_params)

        # Stages 2 to 7 in the original paper Table 1
        self.stages = nn.Sequential(
            *[self.create_stage(*stage_params[i]) for i in range(7)]
        )

        # First layer of stage 9 in the original paper Table 1
        self.pre_head = ConvBnAct(
            scaled_widths[-2], scaled_widths[-1], kernel_size=1, dim=dim
        )
    ...
    @staticmethod
    def create_stage(
        n_in, n_out, num_layers, layer_type, kernel_size, stride, r, p, dim
    ):
        """Creates a Sequential consisting of [num_layers] layer_type."""
        common_params = {"kernel_size": kernel_size, "r": r, "p": p, "dim": dim}
        layers = [layer_type(n_in, n_out, stride=stride, **common_params)]
        layers += [
            layer_type(n_out, n_out, **common_params) for _ in range(num_layers - 1)
        ]
    ...
    def from_name(cls, network_name, **kwargs):
        ...
        config = {
            ...
            # b0 with smaller inputs
            "efficientnet-s": {"width_factor": 1.0, "depth_factor": 1.0, "mode": "s"},
            "efficientnet-xs": {"width_factor": 1.0, "depth_factor": 1.0, "mode": "xs"},
        }
...
class Decoder(nn.Module):
    ...
    > dim  = 2  # can be 2 or 3
    > size = 24
    > encoder = EfficientNet.from_name('efficientnet-s', dim=dim)
    ...
    def __init__(
        ...
        self.conv = nn.Sequential(
            ConvTransposeNd(
                num_features[-1], num_features[-2], dim=dim, **first_conv_params
            ),
            BatchNormNd(num_features[-2], dim=dim),
            nn.ReLU(True),
            ConvTransposeNd(
                num_features[-2], num_features[-3], dim=dim, **other_conv_params
            ),
            BatchNormNd(num_features[-3], dim=dim),
            nn.ReLU(True),
            ConvTransposeNd(
                num_features[-3], out_channels, dim=dim, **other_conv_params
            ),
        )
