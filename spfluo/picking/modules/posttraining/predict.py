...
def load_network(
    ...
    network = EfficientNet.from_name(network, dim=dim, num_classes=num_classes).to(device)
...
def predict_patches_tilt(
    ...
    test_paths_image = list(filter(
        lambda x: '_'.join(os.path.basename(x).split('_')[:-1])+'.'+extension==image, patches_path)
    )
    ...
    (min_patch, _), (max_patch, _) = flatten_patches.min(dim=1), flatten_patches.max(dim=1)
...
def predict_picking(
    ...
    predict_args = [patch_size, dim, network, device, batch_size, stride, predict_on_u_mask]
...
def predict_tilt(
    ...
    predictions = {}
    for image_name in images_names:
        predictions[image_name] = predict_patches_tilt(image_name, test_paths, crop_dir, *predict_args)
