...
def compute_iou_3d(box: np.ndarray, boxes: np.ndarray,
                   box_volume: np.ndarray, boxes_volume: np.ndarray) -> np.ndarray:
...
def spatial_average(grouped_boxes: List[np.ndarray]) -> np.ndarray:
    ...
    for i, group in enumerate(grouped_boxes):
        ...
        mean_boxes[i] = np.array((x1.mean(), y1.mean(), z1.mean(),
                                  x2.mean(), y2.mean(), z2.mean()))
    return mean_boxes
...
def non_max_suppression_3d(boxes: np.ndarray, scores: np.ndarray, threshold: float) -> np.ndarray:
...
def center_of_mass_correction_on_all_bboxes(
    ...
    """
    correction = iterative_center_of_mass_correction if iterative else center_of_mass_correction
...
def postprocess_one_image(
    ...
    # Refinement
    step1, step2, step3 = refine(image, filtered_boxes, iterative, nms_threshold, last_step, margin)
    results = {
        'raw': boxes,
        'nms': filtered_boxes,
